package example.com.jdbc.lab.connection;

import java.sql.*;
import java.util.logging.Logger;

/**
 * Класс ConnectionManager используется для получения и закрытия соединения с
 * базой данных
 */
public class ConnectionManager {
	...
	public static Connection getConnection() {
		if (connection == null) {
			...
			final String url = "jdbc:postgresql://localhost:5432/jdbc_lab_db";
			try {
				connection = DriverManager.getConnection(url, user, password);
				logger.info("Соединение с БД установлено успешно!");
			} catch (SQLException e) {
				closeConnection();
				logger.severe("Соединение с БД установить не получилось: " + e.getMessage());
			}
		}
		return connection;
	}
